...
exports.proxyToBackend = proxyToBackend;
/**
...
async function proxyToBackend(request, path) {
    ...
    if (request instanceof server_1.NextRequest) {
        const cookies = [];
    ...
    try {
        ...
        const nextResponse = responseBody.byteLength > 0 && typeof responseData === 'object'
            ? server_1.NextResponse.json(responseData, { status: response.status, statusText: response.statusText })
            : new server_1.NextResponse(responseData, { status: response.status, statusText: response.statusText });
