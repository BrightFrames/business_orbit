...
class NavigatorService {
    ...
    static getActivityLabel(lastActive) {
        if (!lastActive)
            return 'Unknown';
    ...
    static calculateResponseLikelihood(row) {
        ...
        if (score > 70 && diffDays < 3)
            return 'High';
        if (score < 40 || diffDays > 30)
            return 'Low';
        return 'Medium';
    }
    static interpolateMessage(template, candidate) {
        return template
            .replace(/{{name}}/g, candidate.name.split(' ')[0]) // First name
            .replace(/{{role}}/g, candidate.role);
    }
    static async checkSearchLimits(userId) {
        // Simple daily limit: 20 searches for demo
        const res = await database_1.default.query(`SELECT COUNT(*) FROM navigator_search_logs 
             WHERE user_id = $1 AND created_at > CURRENT_DATE`, [userId]);
