...
async function seed() {
    ...
    `);
        ...
        for (const city of AVAILABLE_CHAPTERS) {
            await database_1.default.query('INSERT INTO chapters (name, location_city) VALUES ($1, $2) ON CONFLICT (name, location_city) DO NOTHING', [`${city} Chapter`, city]);
        }
        ...
        for (const group of DEFAULT_SECRET_GROUPS) {
            await database_1.default.query('INSERT INTO secret_groups (name, description, admin_name) VALUES ($1, $2, $3)', [group.name, group.description, 'Business Orbit Admin']);
        }
