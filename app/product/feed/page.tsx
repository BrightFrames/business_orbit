...
export default function FeedPage() {
  ...
  useEffect(() => {
    if (user) {
      ...
      // Debounced visibility refresh - only if tab was hidden for 2+ minutes
      const VISIBILITY_DEBOUNCE_MS = 120000; // 2 minutes
      let lastVisibilityFetch = Date.now();

      const handleVisibilityChange = () => {
        if (!document.hidden) {
          const now = Date.now();
          if (now - lastVisibilityFetch > VISIBILITY_DEBOUNCE_MS) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-3 sm:py-4 md:py-8 pb-16 sm:pb-20 lg:pb-8">
        <div className="flex flex-col lg:flex-row gap-3 sm:gap-4 lg:gap-8">
          {/* Left Sidebar - Uses CSS for responsive display, components mount ONCE */}
          {/* Mobile Layout - visible on small screens, hidden on lg+ */}
          ...
          {/* Desktop Layout - hidden on small screens, visible on lg+ */}
          ...
          {/* Right Sidebar - Uses CSS for responsive display */}
          {/* Mobile Layout */}
