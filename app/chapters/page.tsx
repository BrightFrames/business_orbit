...
export default function ChapterDashboard() {
  ...
  const fetchTopPerformers = async () => {
    const result = await safeApiCall(
      () => fetch('/api/users/top-performers', { credentials: 'include' }),
      'Failed'
    )
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                          {availableChapters.map((location) => (
                            <button
                              ...
                              className={`p-2 sm:p-3 rounded-lg border-2 transition-all duration-200 text-xs sm:text-sm font-medium cursor-pointer ${selectedChapters.includes(location)
