...
interface OTPCreateResult {
    success: boolean;
    otp?: string; // Only returned for sending via email, NOT stored
    error?: string;
...
export async function verifyOTP(
    ...
    try {
        ...
        // Verify OTP hash
        const isValid = await bcrypt.compare(sanitizedOTP, token.otp_hash);
 ...
 */
export async function invalidateOTPs(email: string, purpose?: OTPPurpose): Promise<void> {
    if (!pool) return;
