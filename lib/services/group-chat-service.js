...
class GroupChatService {
    ...
    async storeMessage(message) {
        ...
        const result = await database_1.default.query(`INSERT INTO secret_group_messages (group_id, sender_id, content)
       VALUES ($1, $2, $3)
       RETURNING id, created_at`, [message.groupId, message.senderId, message.content]);
        const meta = await database_1.default.query('SELECT name, profile_photo_url FROM users WHERE id = $1', [message.senderId]);
        const row = result.rows[0];
        // Award points
        try {
            await (0, rewards_1.awardOrbitPoints)(Number(message.senderId), 'secret_group_activity', 'Active in Secret Group');
        }
        catch (err) {
...
exports.groupChatService = new GroupChatService();
